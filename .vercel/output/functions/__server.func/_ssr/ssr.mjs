...
function applySecurityHeaders(response) {
	...
	headers.set("Permissions-Policy", "camera=(), microphone=(), geolocation=(self), payment=(), display-capture=()");
	const cspDirectives = [
		"default-src 'self'",
...
function sanitizeErrorMessage(err) {
	...
	for (const pattern of SENSITIVE_PATTERNS) if (pattern.test(rawMessage)) return "Terjadi kesalahan pemrosesan pada server. Silakan coba beberapa saat lagi.";
...
async function getServerEntry() {
	if (!serverEntryPromise) serverEntryPromise = import("./server-o1JyFher.mjs").then((n) => n.c).then((n) => n.t).then((m) => m.default ?? m);
...
async function normalizeCatastrophicSsrResponse(response, request) {
	...
	const lastErr = consumeLastCapturedError() ?? /* @__PURE__ */ new Error(`h3 swallowed SSR error: ${body}`);
	logServerError(`SSR Handler Error for ${request.method} ${url}`, lastErr);
	if (isApiOrServerFn) return new Response(JSON.stringify({
		error: "Server Error",
...
var server_default = { async fetch(request, env, ctx) {
	try {
		...
		const url = request.url || "";
		if (url.includes("/_serverFn") || url.includes("/api")) return applySecurityHeaders(new Response(JSON.stringify({
